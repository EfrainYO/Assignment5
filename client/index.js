...
let books = [];
const localHost = 'http://localhost:3333/books';
...
async function handleClickEditButton(bookId) {
  try {
    const buku = await fetch(`${localHost}/${bookId}`);
...
async function deleteBook(bookId) {
  try {
    ...
    */
    // TODO: answer here
    const deleteMybook = await fetch(`${localHost}/${bookId}`, {
      method: 'DELETE',
    });
    if (deleteMybook) {
      console.log(deleteMybook);
    }
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat menghapus buku');
